 ...
 * license: MIT ( [url]https://raw.githubusercontent.com/ForeverEver4/bgpCore/master/LICENSE[/url] )
 * github: [url]https://github.com/ForeverEver4/bgpCore[/url]
 * raw:
 *
 ...
 *
 * author: Four | 作者：Four
 * email: [email]the@four.moe[/email] | 邮箱：[email]the@four.moe[/email]
 */
 ...
 * Math | 数学
 *
 * A extend of origin JavaScript's Math | 对原有JavaScript的数学类的拓展
 **/
 ...
 * String | 字符串
 *
 * A extend of origin JavaScript's String | 对原有JavaScript的字符串类的拓展
 **/
 ...
 * Bitmap | 位图
 *
 * A extend of origin Bitmap | 对原有位图类的拓展
 **/
 ...
 * SceneManager | 场景管理器
 *
 * A extend of origin SceneManager | 对原有场景管理器类的拓展
 **/
...
CharacterManager.prototype = Object.create(CharacterManager)
CharacterManager.prototype.constructor = CharacterManager
...
CharacterManager.getRealCellPosition = function (characterId) {
    ...
    return new Point(character._realX, character._realY)
}
 
 
 
/**
 * A static class use for manage events & event data | 用于管理事件和事件数据的类
 *
 * @static
 **/
...
EventManager.getMapEventsMatchNote = function (note, mode) {
    ...
    EventManager.forMapEvents(function(eventId){
        isMatchNote(eventId) ? matchEvents.push(eventId) : 0
    })
    return matchEvents
}
/**
