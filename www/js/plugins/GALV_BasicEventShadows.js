...
Galv.BES.opacity = Number(PluginManager.parameters('Galv_BasicEventShadows')["影子透明度"]);
	...
	if (this.event().note.contains("<shadow>") && this._characterName != "") {
		...
		if (page) {
			for (var i = 0; i < page.list.length; i++) {
				...
				if (page.list[i].code == 108 && page.list[i].parameters[0].contains("<shadow-48>")) {
					var adjusty = true
				};
			};
		};
...
Sprite_BasicShadow.prototype.update = function() {
	...
	if (this._character._characterName == "" || this._character._transparent) {
		...
		this.opacity = Galv.BES.opacity//this._character._opacity
	};
};
