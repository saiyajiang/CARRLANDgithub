...
var sxwolf = PluginManager.parameters('qabs')
var attackspeed = String(sxwolf[ '攻击速度关键字'||attackwait]);
var attackspeedvariables = String(sxwolf['攻击速度变量'||41]);
var attackspeedad = Number(sxwolf['攻击速度参照值'||50]);

var castspeed = String(sxwolf[ '吟唱速度关键字'||castwait]);
var castspeedvariables = String(sxwolf['吟唱速度变量'||57]);
var castspeedad = Number(sxwolf[ '吟唱速度参照值'||50]);

var movespeedflag = String(sxwolf[ '移动速度关键字'||movespeed]);
var movespeedvariables = String(sxwolf['移动速度变量'||56]);
var movespeedad = Number(sxwolf['移动速度参照值'||100]);
var movespeeddefault = Number(sxwolf['基础移动速度'||4]);

var jumpawayfukong = String(sxwolf['浮空关键字'||fukong]);
var jumpawayfukongactor = String(sxwolf['角色浮空关键字'||fukongactor]);
...
(function() {
  ...
  Skill_Sequencer.prototype.startAction = function(action) {
    ...
    switch (cmd) {
	  ...
	  //
	  case attackspeed: {
        this.actionWaitattack(action);
        break;
      }
	  case castspeed: {
        this.actionWaitcast(action);
        break;
      }
	  case 'waitzhongji': {
        this.actionWaitzhongji(action);
        break;
      }
	  case 'moveyuanhujianqi': {
        this.actionMoveyuanhujianqi(action);
        break;
      }
	  case 'movepomiezhan': {
        this.actionMovepomiezhan(action);
        break;
      }
	  case 'moveyaoxingzhan': {
        this.actionMoveyaoxingzhan(action);
        break;
      }
	  case 'movehuahuo': {
        this.actionMovehuahuo(action);
        break;
      }
	  case 'waitquanjishu': {
        this.actionWaitquanjishu(action);
        break;
      }
	  case 'waitjufengquan': {
        this.actionWaitjufengquan(action);
        break;
      }
    }
  };
  ...
  Skill_Sequencer.prototype.startUserAction = function(action) {
    ...
    switch (cmd) {
	  ...
	  //自定义属性
	  case movespeedflag: {
        this.userSpeedmove(action);
        break;
	  }
	  case 'jumpdistance': {
        this.userJumpdistance(action);
        break;
	  ...
	  case 'speedchongfeng': {
        this.userSpeedchongfeng(action);
        break;
	  }
	  case 'speedzhuifeng': {
        this.userSpeedzhuifeng(action);
        break;
	  }
	  case 'speedliediquan': {
        this.userSpeedliediquan(action);
        break;
	  }
	  case 'speedjufengquan': {
        this.userSpeedjufengquan(action);
        break;
	  }
    }
  };
  ...
  Skill_Sequencer.prototype.startOnDamageTargetAction = function(action, targets) {
    ...
    switch (cmd) {
	  ...
	  //自定义属性
	  case jumpawayfukong: {
        this.targetJumpawayfukong(action, targets);
        break;
		
      }
	  case jumpawayfukongactor: {
        this.targetJumpawayfukongactor(action, targets);
        break;
	  }
    }
  };
...
/////////////////////////////////////////////////////////////////////////////////////////
  ...
  Skill_Sequencer.prototype.actionMoveSkill = function(distance, duration) {
	...
	//弹道距离加成
    this._skill.newX = this._skill.collider.x + Math.round(distance*(1+$gameVariables.value(54)/100) * Math.cos(this._skill.radian));
    this._skill.newY = this._skill.collider.y + Math.round(distance*(1+$gameVariables.value(54)/100) * Math.sin(this._skill.radian));
...
//攻击属性动画
 ...
 //武器类型动画
  ...
  //攻速加成
   ...
   //吟唱速度
    Skill_Sequencer.prototype.actionWaitcast = function(action) {
    var duration = Number(action[0])*(castspeedad/($gameVariables.value(castspeedvariables)+castspeedad));
    ColliderManager.draw(this._skill.collider, duration);
    this._waitCount = duration;
  };
  ...
  // };
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      targets[i].setDirectionFix(lastDirectionFix);
      targets[i].setRadian(prevRadian);
	  targets[i].setMoveSpeed(3);
    }
  };

    Skill_Sequencer.prototype.targetJumpawayfukongactor = function(action, targets, height) {
    var dist = Number(action[1]) || 0;
    for (var i = 0; i < targets.length; i++) {
...
(function() {
  ...
  Game_BattlerBase.prototype.addNewState = function(stateId) {
    ...
    if ($dataStates[stateId].meta.moveSpeeduser) {
      this._moveSpeed += Number($dataStates[stateId].meta.moveSpeeduser+$gameVariables.value(56)) || 0;
    }
...
(function() {
  ...
  Game_Battler.prototype.moveSpeeduser = function() {
    return this._moveSpeeduser;
  };
...
(function() {
  ...
  Game_CharacterBase.prototype.forceSkill = function(skillId, forced) {
    ...
    this._activeSkills.push(skill);
	var cd = Math.floor(skill.settings.cooldown * (100/($gameVariables.value(42)+100)));
	
    this._skillCooldowns[skillId] = cd <= 0?0:cd.toFixed(2);
    ColliderManager.draw(skill.collider, skill.sequence.length + 60);
    return skill;
  };
